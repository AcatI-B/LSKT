...
# python scripts/train.py -m LSKT -d [assist09,assist17,algebra05,statics] -bs 32 -tbs 32 -p -cl --proj [-o output/LSKT_assist09] [--device cuda]
    ...
    if max_memory_free > 1800:
         if mode=='stage1':
               # 执行命令
               subprocess.run(['python', '/home/q22301200/current/KT_module/LSKT-main/scripts/test.py',
                                '-m', 'LSKT',
                                '-d',f'{choice}',
                                '-bs', '16',
                                '-tbs','16',
                                '-p',
                                '-o',f'/home/q22301200/current/KT_module/LSKT-main/{choice}_result',
                                '--device',f'cuda:{gpu_id}',
                                # '--device',f'cuda:2',
                                # '--device','cpu',
                                '-f','/home/q22301200/current/KT_module/LSKT-main/assist09_result/model-010-0.8369.pt',
                                '-emb','3pl'
                                ]) 
